...
const App = () => {
    ...
    // Inventory & UI State
    const [inventory, setInventory] = useState([]); // Admin's full inventory
    const [userInventory, setUserInventory] = useState([]); // Normal user's assigned inventory
    ...
    // CRITICAL FIX: Computed inventory based on user role
    const displayInventory = is_admin ? inventory : userInventory;
    ...
    const fetchUserInventory = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${API_BASE_URL}/api/my-inventory`);
    ...
    // CHART EFFECT: Now uses 'displayInventory'
    useEffect(() => {
        if (chartRef.current && displayInventory.length > 0) {
            if (chartInstanceRef.current) {
                chartInstanceRef.current.destroy();
            }
            const ctx = chartRef.current.getContext('2d');
            const labels = displayInventory.map(item => item.name);
            const data = displayInventory.map(item => item.quantity);
    ...
    }, [displayInventory]); // Dependency is the computed inventory

    const filteredAndSortedInventory = inventory // Only Admins use this list
    ...
    const DashboardView = () => (
        <>
            ...
            {/* Show Summary Metrics ONLY for Admin */}
            {is_admin && (
            ...
            {/* Chart View (Uses displayInventory) */}
            <div className="bg-gray-800 p-6 rounded-xl shadow-lg mt-8">
                <h5 className="text-xl font-medium text-center text-gray-100 mb-4">
                  {is_admin ? "Inventory Quantity Breakdown (All Stock)" : "Your Allocated Items Breakdown"}
                </h5>
                <div className="relative h-96">
                    {displayInventory.length > 0 ? (
                        ...
                        <div className="text-center text-gray-400 p-4">
                            {is_admin 
                                ? "Add items to see the chart!" 
                                : "No items have been allocated to you yet."
                            }
                        </div>
                    )}
                </div>
    ...
    const renderContent = () => {
        if (!is_admin) {
            ...
            switch (currentPage) {
                case 'my-inventory':
                    // Pass the userInventory state to the UserInventoryView component
